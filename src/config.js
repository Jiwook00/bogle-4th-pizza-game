...
export const ROUNDS = [
  {
    ...
    // 코치 시퀀스 — 탭으로 넘김. finger:true 인 비트에서 고스트 손가락 데모.
    // 마지막 비트가 끝나면 입력이 열리며 실전(연습) 시작.
    coach: [
      {
        text: "칼질은 간단해. 피자 위를 손가락으로 쓱— 그으면 그 자리가 갈라져.",
        finger: true,
      },
      { text: "대신 칼은 직선만 돼. 곡선으론 못 갈라 — 한 번에 쭉 밀어." },
      { text: "자, 몸 풀기다. <b>2조각</b>으로 딱 잘라봐. 되면 완성 눌러!" },
    ],
    // 연습 종료 후 결과 카드 대신 이어지는 코치 대사(분기 + 공통 마무리).
    // 분기: n<주문=short, n>주문=over, 공평도<unevenThreshold=uneven, 그 외=good.
    unevenThreshold: 75,
    coachOutro: {
      good: "오, 깔끔한데? 딱 반으로 갈랐어.",
      uneven: "음, 한 쪽이 좀 크네. 실전에선 최대~한 똑같이!",
      short: "어라, 아직 제대로 안 갈랐잖아? 뭐, 감은 잡았지?",
      over: "하하, 좀 많이 갈랐네. 2조각이면 충분해.",
      end: "좋아, 연습은 여기까지! 이제 진짜 손님들이야 — 시간도 잰다. 준비됐지?",
    },
  },
  {
    id: "r1",
